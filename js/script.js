(function ($, window) {
    ...
    var App = App || {
        init: function () {
            ...
            $('.js-search .text-input').keydown(function (event) {
                if (event.keyCode == 13) {
                    location.href = 'https://www.google.com/search?q=site:www.yunui.com ' + $(this).val();
                    return false;
                }
            })
        }
    ...
    $(function(){
        ...
        if(bannerNode.data('enable')){
            ...
            bannerNode.attr('style','background-image:url(/banner/'+index+'.png)');
            bannerNode.attr('style','background-image:url(/banner/'+index+'.jpeg)');
